...
module.exports = function (gaKey) {
	...
	function passwordReset (response, auth, db, callback) {
		if (auth.isAdmin()) {
			var users = require('./users')(db);
			var data = { auth: auth };
			users.getAllUsers(function(e, users){
				...
				data.users = users;
				renderPage(response, 'passwordReset', data);
			});
		}
		else{
			return indexRedirect(response);
	...
	function passwordResetResult (response, auth, db, userId, newPassword, onError) {
		if (auth.isAdmin()) {
			var users = require('./users')(db);
			var data = { auth: auth };
			users.getUserById(userId, function(e, user){
				...
				data.user = user;
				renderPage(response, 'passwordResetResult', data);
			});
		}
		else{
			return indexRedirect(response);
	...
	function predictionsReport(response, auth, db, callback, fromDate, format, requestedMiniseason, onlyAcceptedUsers) {
		if (auth.isAdmin()) {
			var users = require('./users')(db);
			var fixtures = require('./fixtures')(db);
			var data = {
				auth: auth,
				onlyAcceptedUsers: onlyAcceptedUsers ? true : false
			};
			users.getAllUsers(function(e, users){
				if (e) return callback(e);
						...
						if (season.current) {
							...
							for (var x=0; x<season.miniseasons.length; x++) {
								...
								for (var y=0; y<users.length; y++) {
									...
									if (onlyAcceptedUsers && !user.accepted) continue;
									if (!user.predictions || 
	...
	return {
		...
		changeMyPassword: function (response, data) {
			renderPage (response, 'changePassword', data);
		},
