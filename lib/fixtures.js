...
function getCurrentSeason(db, onError, onSuccess, initialise) {
	db.view('fixtures/currentSeason', {}, function (e, doc){
		if (e) return onError(e);
		if (doc.length == 0) {
			onSuccess(initialise ? newSeason() : null);
		}
		else {
...
module.exports = function(db) {
	return {
		getCurrentSeason: function(onError, onSuccess, init) {
			getCurrentSeason(db, onError, onSuccess, init);
		},
			...
			if (seasonData.id) {
				getSeasonById(db, seasonData.id, onError, function(season){
					var activate = function () {
						...
						db.save(season, function(e, result){
							if (e) return onError(e);
							var message = 'Game ' + season.name + ' now active.';
