...
module.exports = function(db, session) {
	return {
		...
		setLoginCookie: function(data, response, callback) {
			getUserByUsername(db, data.username, function(user){
				user.loginToken = uuid.v1(); 
		...
		doAutoLogin: function(data, onSuccess, onError) {
			getUserByUsername(db, data.username, function(user){
				if (user.loginToken == data.token) {
					...
					onError('Login token incorrect. Likely someone\'s hacked your account bro.');
				}
			});
		}
	};
