...
module.exports = function(session, users) {
	...
	return {
		...
		changePassword: function (data, onSuccess, onFailure) {
			var username = session.user.username
			  , current = data.current
			  , password = data.password
			  , repeat   = data.repeat;

			if (!current || !password || !repeat) return onFailure('Current password and new password (twice) are required.');
			if (password != repeat) return onFailure('New passwords don\'t match.');
			if (password.length < 6) return onFailure('The new password is a shitty password.');
			if (password == username) return onFailure('Password can\'t be the same as Username. You dolt.');
			if (password.toLowerCase() == 'password') return onFailure('Don\'t use \'' + password + '\' as your password. Are you slow?');
			if (password.toLowerCase() == 'password1') return onFailure('Don\'t use \'' + password + '\' as your password. Are you slow?');
			if (password.toLowerCase() == 'password123') return onFailure('Don\'t use \'' + password + '\' as your password. Are you slow?');

			users.getUserByUsername(username, onFailure, function(user){
				if (bcrypt.compareSync(current, user.password))
				{
					users.changePassword(user, password, function(e) {
						if (e) return onFailure (e);
						onSuccess('Password updated OK.');
					});
				}
				else {
					return onFailure('The current password you entered isn\'t right.');
				}
			});
		}
	};
